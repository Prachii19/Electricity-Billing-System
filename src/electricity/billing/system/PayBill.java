...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter){
        ...
        try{
            ...
            rs=con.stmt.executeQuery("select * from bill where meter_no='"+meter+"' and month='January'");
            while(rs.next()){
        ...
        cmonth.addItemListener(new ItemListener(){
           public void itemStateChanged(ItemEvent ae){
                try{
            Conn con=new Conn();
            ResultSet rs=con.stmt.executeQuery("select * from bill where meter_no='"+meter+"' and month='"+cmonth.getSelectedItem()+"'");
            while(rs.next()){
                labelunits.setText(rs.getString("units"));
                labeltotalbill.setText(rs.getString("totalbill"));
                labelstatus.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==pay){
            try{
                Conn con=new Conn();
                con.stmt.executeUpdate("update bill set status='Paid' where meter_no='"+meter+"'and month='"+cmonth.getSelectedItem()+"'");
            }catch(Exception e){
                e.printStackTrace();
            }
            setVisible(false);
